...
with st.container():
    ...
    with user_col_1_2:
        user_age = st.number_input(
            "How old are you?", min_value=18, max_value=120, value=30
        )
...
with st.container():
    st.markdown(
        '### Please answer the following questions and then click the *"Start prediction"* button.'
    )
    ...
    with select_col_6_1:
        varRENTHOM1 = st.selectbox(
            "Do you own or rent your home?", ("Own", "Rent", "Other arrangement")
        )
    with select_col_6_2:
    ...
    with select_col_8_2:
        var_URBSTAT = st.selectbox(
            "Urban/Rural Status", ("Urban counties", "Rural counties", "Not Sure")
        )
    ...
    with num_col_1_1:
        varMENTHLTH = st.number_input(
            "For how many days during the past 30 days was your mental health not good",
            min_value=0,
            max_value=30,
        )
    with num_col_1_2:
        varPOORHLTH = st.number_input(
            "During past 30 days, for how many days did poor physical or mental health keep you from doing usual activities?",
            min_value=0,
            max_value=30,
        )
    num_col_2_1, num_col_2_2 = st.columns(2)
    with num_col_2_1:
        varPHYSHLTH = st.number_input(
            "For how many days during the past 30 days was your physical health not good?",
            min_value=0,
            max_value=30,
        )
    with num_col_2_2:
        varFALL12MN = st.number_input(
            "In the past 12 months, how many times have you fallen? (Enter 77 if not sure)",
            min_value=0,
            max_value=77,
        )
    num_col_3_1, num_col_3_2 = st.columns(2)
    with num_col_3_1:
        varSLEPTIM1 = st.number_input(
    ...
    with num_col_3_2:
        varALCDAY5 = st.number_input(
            "During the past 30 days, how many days did you have at least one drink of any alcoholic beverage?",
            min_value=0,
            max_value=30,
        )
...
light_GBM_model = ProbabilisticBinaryClassifier(
    joblib.load("models/LGBMClassifier-testing-2022-07-09_19_04_48.pkl"),
    prob_threshold=None,
)
